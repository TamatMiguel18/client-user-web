...
export const AlertCard = ({ alert, onSelect }) => {
  ...
  return (
    <Card hover className="flex flex-col h-full border-t-4 rounded-2xl border-slate-800 hover:border-red-500/30 transition-all shadow-lg">
      ...
      <h3 className="text-xl font-bold mb-1 text-slate-200">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <div className="bg-black/30 p-2 rounded flex items-center gap-2">
          <Thermometer size={14} className="text-orange-400" /> {alert.temperature ?? '--'}°C
        </div>
        <div className="bg-black/30 p-2 rounded flex items-center gap-2">
          <Droplets size={14} className="text-blue-400" /> {alert.humidity ?? '--'}%
        </div>
      </div>
      ...
      <button
        ...
        className="w-full py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm font-semibold transition-all"
      >
        Ver Detalle
      </button>
    </Card>
